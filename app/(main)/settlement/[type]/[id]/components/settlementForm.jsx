...
export default function SettlementForm({ entityType, entityData, onSuccess }) {
  ...
  const handleUserSettlement = async (data) => {
    ...
    try {
      ...
      let newNetBalance = entityData.netBalance;
      if (data.paymentType === "youPaid") {
        newNetBalance -= amount;
      } else {
        newNetBalance += amount;
      }
  ...
  const handleGroupSettlement = async (data, selectedUserId) => {
    ...
    try {
      ...
      // Calculate new balances for the group
      const updatedBalances = entityData.balances.map((balance) => {
        if (balance.userId !== selectedUserId) return balance;

        let newBalance = balance.netBalance;
        if (data.paymentType === "youPaid") {
          // You paid them - you owe them less (negative balance decreases)
          newBalance -= amount;
        } else {
          // They paid you - they owe you less (positive balance decreases)
          newBalance += amount;
        }
      ...
      toast.success("Settlement recorded successfully!");
      if (onSuccess) onSuccess(updatedBalances);
    } catch (error) {
      toast.error("Failed to record settlement: " + error.message);
    }
  };
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    >
                      {isOwing
                        ? `You Owe Them $${Math.abs(member.netBalance).toFixed(2)}`
                        : isOwed
